...
**Confirmado em duas frentes independentes — inspeção manual do artifact baixado E o gate automatizado corrigido** (`at3-ffmpeg-build-6`, run [29381974380](https://github.com/luistiagos/ominitranslator/actions/runs/29381974380), 5/5 gates verdes com a âncora real): FFmpegKitNext v8.1.0, arm64-v8a only, API 28. A linha real de `./configure`, extraída via `strings` do `libavutil.so` compilado (não da árvore-fonte), contém `--enable-version3 --enable-libopenh264` e **nenhuma ocorrência de `--enable-gpl`** — LGPL v3, sem GPL, com openh264 conforme decisão C3. AAR (17,8 MB) com 10 `.so` arm64-v8a, todas com `LOAD Align = 0x4000` (16 KB), verificado com `llvm-readelf -lW` localmente (não só no CI).

**Por que LGPL v3 e não v2.1:** não é uma escolha do nosso build — `scripts/android/ffmpeg.sh:443` do próprio `arthenica/ffmpeg-kit-next` passa `--enable-version3` **incondicionalmente** para todo build Android, independente de qualquer flag nossa (a flag GPL, por comparação, só entra `if [ "$GPL_ENABLED" == "yes" ]`, linha 361-363 do mesmo arquivo — nunca setada aqui). O próprio wrapper já se declara "License LGPLv3.0" (`scripts/function.sh:713`, saída do `--version`). Sem efeito prático sobre nossas obrigações de distribuição: para linkagem dinâmica de um `.so` num app proprietário, LGPLv2.1 e v3 pedem a mesma coisa (avisos de licença preservados, possibilidade de religar/substituir a lib, oferta do fonte *da lib*) — o AAR já traz isso resolvido (`res/raw/license.txt`, `license_openh264.txt`, `source.txt`), exatamente o que a §16.3 da spec pede para preservar.

**Histórico de diagnóstico (6 execuções; nenhuma delas foi o FFmpeg falhando — sempre bug no meu script de evidências; histórico completo em `decisoes.md`):**
- **build-1/2**: coleta de evidências não achava os caminhos certos no layout do `nix-build` da v8.1.0. Corrigido: coleta *best-effort* (nunca falha) + gates movidos para depois do upload do artifact.
- **build-3/4**: gate de alinhamento 16 KB reprovava por **falso positivo**: `readelf -l` (sem `-W`) quebra cada `LOAD` em duas linhas e a coluna `Align` fica na segunda; o grep só via a primeira. Corrigido com `readelf -lW` + checagem de `$NF`.
- **build-5**: todos os 5 gates passaram — mas ao **baixar o artifact e inspecionar manualmente**, `buildconf.txt` continha só *format strings* do código-fonte do FFmpeg (ex. `av_log(..., "%s%-11s configuration: %s\n", ...)`), não a linha real de configure. Os Gates 2/3 (licença) **passavam sem checar nada de verdade** — deram sorte de o texto-lixo não conter `--enable-gpl` por coincidência. **Corrigido (build-6) extraindo a configuração real de dentro do binário compilado** (`strings libavutil.so | grep -- --target-os=`, verificado localmente com `llvm-strings` contra o `.so` já baixado antes de subir) e apertando o Gate 2 para exigir a âncora `--target-os=`, não só "arquivo não-vazio".
- Efeito colateral útil, reaproveitável em outros workflows: download de log/artifact exige auth admin, mas a **listagem** de artifacts e as **anotações `::error::`** de um repo público são legíveis anonimamente via API — os gates passaram a ecoar o conteúdo real do erro, permitindo diagnosticar 4 das 6 iterações sem token nem UI web (as duas restantes exigiram baixar e inspecionar o artifact manualmente, que é exatamente o tipo de coisa que só aparece inspecionando o artefato real, não só a saída de "passou/falhou").

**Lição para qualquer gate baseado em CI:** "o step não falhou" não é a mesma prova que "o step checou o que devia" — sempre que uma evidência crítica (aqui, prova de licença) sai de um `grep` amplo, vale desconfiar de falso-positivo por coincidência de texto e, pelo menos uma vez, inspecionar o artefato final manualmente antes de confiar no gate automatizado.

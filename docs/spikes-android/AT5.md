...
- **Resolvido por completo (D3.4 + fix de 2026-07-17)** — `models_screen.dart` mostra "instalando..." quando a extração `.tar.gz` está em andamento, em vez de uma barra parada sem explicação; e o bloqueio de UI em si (extração síncrona no isolate principal, que já disparou ANR real no smoke da D3.4 para os modelos maiores — Whisper 61MB, vozes pt-BR/es 59-72MB) foi corrigido movendo a extração pra `Isolate.run` (`model_manager.dart:1007`). Validado no device sem ANR mesmo sob input concorrente ativo. Detalhes em `decisoes.md` (2026-07-17).

...
- ~~Melhorias de produção derivadas do AT-2b: (a) extração por streaming, (b) `espeak-ng-data` compartilhado~~ — **infraestrutura feita** (2026-07-14): `ModelManager` ganhou `kind: 'targz'` (extração via `extractFileToDisk` do `package:archive`, streaming por `InputFileStream`/`OutputFileStream` em vez de `decodeBytes`) e `ModelEntry.dependsOn` + `ModelCatalog.resolveRequiredIds()` (dependência compartilhada, deduplicada, segura contra ciclo). Testado com fixtures sintéticas (7 testes novos, `model_manager_test.dart`) — **não aplicado** às 58 vozes reais do catálogo Windows nem a um `ModelCatalog.android()` (que ainda não existe e não deve ter URLs inventadas). Efeito colateral: achado e corrigido um bug real e pré-existente — `stateOf()`/`download()` liam o manifest **estático** do Windows (`manifest.firstWhere`) em vez do `catalog` da própria instância, então qualquer `ModelManager` com catálogo customizado já estava quebrado nessas duas operações; hoje usam `catalog.entryOf(id)`.

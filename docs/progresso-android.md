...
**Achado de produção sem fix nesta passada — pendência de severidade alta, documentada e não bloqueante:** `ModelManager.download()` extrai `.tar.gz` de forma síncrona em Dart puro no isolate principal. Para modelos grandes (Whisper "Rápido" 61MB, vozes pt-BR/es 59MB/72MB) isso bloqueia a UI por ~60-180s, tempo suficiente pra disparar o diálogo "app não está respondendo" do Android quando há input concorrente durante a extração. O rótulo "instalando..." (`models_screen.dart`, abaixo) evita a barra travada em 100% sem explicação, mas não resolve o bloqueio — precisa de `compute()`/isolate dedicado, escopo de fase futura.

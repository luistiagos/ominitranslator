...
- **Fecha a confirmação de runtime que o AT-0 tinha deixado pendente** (§16.1): `zipalign -c -P 16 -v 4` no APK real → **Verification succesful**; `readelf -lW` em todas as 5 `.so` → **todos os `LOAD` com `Align` múltiplo de 16 KB** (sherpa/ORT em `0x4000`; `libflutter.so` e a lib de validação Vulkan em `0x10000` — mais rígido, também compatível: 65536 = 4×16384). Reconfirmado na P2 da revisão de 2026-07-16 com `libslimt.so` real dentro do APK (empacotado via `jniLibs`, ver §3.3j/P2): também `Align=0x4000` nos 3 `LOAD`. Só falta o teste num emulador/device Android 15+ de página 16 KB **de verdade em runtime** (o g86 é 4 KB e não exercita isso — todas as confirmações acima são estáticas, via `readelf`, não um boot real em kernel 16 KB).
  - **Rotas reais para fechar isso (P5 da revisão de 2026-07-16):** (a) a developer option "Boot with 16 KB page size" — **checada em 2026-07-17: o moto g86 NÃO a expõe** (Android 16, kernel 6.1 `android14-11`, página 4KB; nenhuma prop `*16k*`, setting `enable_16k_pages` nula — a option exige o OEM shippar a variante de kernel 16KB, e a Motorola não shippou pro g86); (b) um device físico com kernel 16 KB nativo (nenhum disponível agora); (c) build x86_64 dos nativos só para esse teste específico — custo alto (recompilar sherpa/ONNX Runtime/slimt/FFmpegKitNext pra x86_64), última opção. Um AVD x86_64 de 16 KB **não serve**: os nativos do projeto são `arm64-v8a`-only (`abiFilters`), e rodar um AVD arm64 num host Windows x86 é inviável (sem aceleração de hardware).
...
- Smoke test funcional no device — **RESOLVIDO em 2026-07-17 por caminho mais forte**: o `AndroidTranslator` de produção (FFI + C-API dos patches 0003/0004, a MESMA `libslimt.so` da Release) traduziu 5/5 frases en→pt corretamente no moto g86 em 436ms. A comparação `slimt-cli` standalone contra o SA-1 ficou dispensada (ver `slimt-build.md` §6).
...
- Smoke test funcional no device — **PASSOU em 2026-07-17 (moto g86), 4/4**: catálogo real (5 modelos, download+hash+extração; whisper 182,8s — gargalo é a extração do tar.gz em Dart puro), `AndroidTranslator` 5/5 en→pt em 436ms, `AndroidTranscriber` com o fix do B1 confirmado em runtime (asr_in.wav do tamanho exato; 8 segmentos perfeitos em 4,6s, timestamps casando os gaps da fixture), `AndroidSynthesizer` 2,36s de pt-BR em 422ms (WAV validado no desktop). **Achado de produção no processo**: o AAR local não resolve a transitiva `smart-exception-java` do POM — crash em runtime no primeiro uso do FFmpegKit; corrigido no `build.gradle.kts` (ver `decisoes.md` 2026-07-17). `tool/verify.ps1` verde: engine com 355 testes (+21 — 13 `AndroidTranslator`, 8 `FFmpegKitNextRunner`), mais 3 novos no app (`android_ffmpeg_test.dart`).
...
10 testes novos no engine (368 no total) + 11 no app (`media_processing_service_test.dart`: contrato dos canais + `getJob`/`listRecoverableJobs`/`exportJob` contra um `FileJobCheckpointStore` real via `PathProviderPlatform.instance` mockado). Harness de validação versionado em `tool/android/smoke/smoke_service_main.dart` (mesmo padrão de swap temporário do `main.dart`, nunca commitado) com o roteiro manual completo dos 5 cenários do §14.6 no cabeçalho. Wiring da UI de produção (`AppState`/`home_screen.dart`/`progress_screen.dart` falando com o serviço) é escopo explícito da D3.4, não desta fase.

**Validação on-device (2026-07-17, moto g86) — PASSOU.** O serviço **dublou um vídeo real de ponta a ponta** (28,5s de fala en sintética → pt): eventos ao vivo fluindo com os estados corretos pós-auditoria (`imported` durante o demux, `demuxed` durante a transcrição), `isForeground=true types=0x2000` confirmado via dumpsys (API 36 → ramo mediaProcessing do gate A5), e a saída validada no desktop — MP4 h264 + 2 faixas AAC (dublagem + original), 2 SRTs e `sync.json` com **8/8 segmentos em ±300ms (100%, worstDelta=0ms)**. Os 5 cenários do §14.6: **(a)** tela apagada e **(b)** troca de app — o job continuou e COMPLETOU nesses estados; **(c)** rotação — não recria a Activity neste app (manifest `configChanges`); a recriação real veio do **(d)** "Não manter atividades" + HOME: Activity destruída com o job rodando, o job completou sem Activity nenhuma e a UI reaberta reconectou ao estado persistido; **(e)** `am force-stop` no meio de um job — processo novo listou 4 jobs recuperáveis, o interrompido com o estado parcial `synthesized` gravado na última transição (retomada reinicia do zero — a lacuna documentada do MVP, inalterada). **Bug de produção achado pelo smoke**: o `DartEntrypoint` de 2 argumentos não resolve `serviceMain` (vive em `src/service_entrypoint.dart`, não na biblioteca raiz) — corrigido com o construtor de 3 argumentos (library URI); e o `GeneratedPluginRegistrant.registerWith` explícito era redundante (o construtor do engine auto-registra). Detalhes em `decisoes.md` 2026-07-17.

**Auditoria pós-D3.3 (2026-07-16, pedida pelo usuário) — 9 achados, todos corrigidos no mesmo dia.** Os que mudam comportamento: **(A1)** `jobStateForStage` gravava o estado de conclusão do estágio no primeiro evento DELE (`transcribed` quando a transcrição começa) — corrigido pra gravar o estágio CONCLUÍDO anterior, que é o que os nomes do §9.1 significam e o que o `resolveResumeState` vai precisar; **(A2)** o `ffmpegPoll` do serviço tinha regredido vs o snippet do F4 (`logsTail` sempre vazio → falha de ffmpeg no device viraria erro cego; `statTimeMs` omitido); **(A5, decisão do usuário)** `mediaProcessing` só existe na API 35 e o Android 14 valida tipos de FGS — fallback `dataSync` pra API 29–34 (manifest `mediaProcessing|dataSync` + permissão `FOREGROUND_SERVICE_DATA_SYNC`, desvio documentado do §13.3); **(A6)** handshake `workerReady` Dart→Kotlin elimina a dependência do ChannelBuffers (capacidade 1) pra `runJob` disparado durante o boot do isolate, e os handlers Kotlin passaram a ser registrados antes do `executeDartEntrypoint`; **(A9)** o `onListen` do EventChannel religa ao serviço vivo — Activity recriada com job em andamento voltava a receber eventos ao vivo (§14.6); `BIND_AUTO_CREATE` de propósito (binding flags=0 deixaria `mediaService` como referência morta após o `stopSelf`, porque Binder local nunca dispara `onServiceDisconnected` em destruição graciosa). Mais: PendingIntent da notificação sem `FLAG_ACTIVITY_NEW_TASK` (A3), início do `_runJob` fora do try — falha precoce deixaria o serviço em foreground pra sempre (A4), guard de `runJob` concorrente no isolate (A7), `exportJob` só flipa de `completedPendingExport` + notificação usa a mensagem humana do evento (A8). Detalhes completos em `decisoes.md` (2026-07-16). 369 testes no engine, 12 no app; verify verde, APK rebuilda, manifest mesclado reconferido (`foregroundServiceType=0x2001`).

### 3.3n D3.4 — UI real ligada ao `MediaProcessingService` — **PASSOU** no moto g86, fecha o gate da D3

Última sub-fase da D3: a UI de produção (`app_state.dart`, `home_screen.dart`, `progress_screen.dart`, `main.dart`, `settings.dart`), até aqui 100% desktop, passou a falar com o serviço real escrito na D3.3. `AppState` ganhou um branch `Platform.isAndroid` em `startJob`/`cancelJob`/construtor (`_startAndroidJob` novo, `currentJobId` novo) — o caminho desktop (isolate) ficou byte-a-byte intocado.

**Duas lacunas reais no protocolo da D3.3, achadas relendo o código antes de escrever a UI (a auditoria anterior não tinha pego):**

- `service_entrypoint.dart` chamava `runDubbingJob(...)` **sem `onDone:`** — o `DubbingResult` do pipeline era descartado sempre. Corrigido (`onDone: (r) => result = r`); `DubbingResult` ganhou `toJson()/fromJson()` (mesmo padrão do `DubbingJobConfig`) trafegando em `extra: {'result': ...}` no evento `jobCompleted`.
- o payload do evento só carregava o `JobState` derivado (com perda proposital da auditoria da D3.3), mas `progress_screen.dart` renderiza por `PipelineStage` cru — sem o stage, a tela não tinha como funcionar no Android. Corrigido com `extra: {'stage': event.stage.name}`, reconstruído do lado `AppState` num `PipelineEvent` real — **zero mudança em `progress_screen.dart`**.

**Risco de arquitetura fechado por decisão:** `pipeline.dart` apaga o `workDir` de um job bem-sucedido logo após o mux, antes do `onDone` — se o Android usasse a mesma raiz pra `workDirBase` e pra `jobsRootDir()` (onde mora `job.json`), um job concluído apagaria o próprio checkpoint. Três raízes distintas sob `appRootDir()`: `jobsRootDir()` (checkpoints), `workRootDir()` (scratch do pipeline, novo default de `AppSettings.workDirBase` no Android) e `outputsRootDir()` (saída final, sobrevive à limpeza do workDir).

**Import/export SAF:** import (`pickImportDocument`+`copyUriToLocalFile`) em `home_screen.dart`, antes de montar o `DubbingJobConfig`; export (`pickExportLocation`+`copyLocalFileToUri`+`exportJob`, fechando o D-2 da D3.3 — só aqui o checkpoint vira `exported` de verdade) em `progress_screen.dart` (`_exportOutput`, botão "Salvar em...").

**UI escondida no Android** — correção de plataforma, comprovada por `pipeline.dart` (`createDownloader`/`createDiarizer` sempre nulos no M1), não polimento: campo de YouTube+cookies, botões "Alterar" de diretório de trabalho e destino de saída (`FilePicker.getDirectoryPath()` devolve `content://` incompatível no Android — o destino final agora é escolhido no export, não antes do job), dica de diarização. `_cleanupOldWorkDirs()` virou condicional a `!Platform.isAndroid` (o foreground service pode manter um job vivo entre recriações de `AppState`; reconstruir apagaria o `workDir` de um job em andamento — pendência de acúmulo documentada, não regressão).

**Validação real no device (moto g86, 2026-07-17) — en→pt e en→es completos pela UI de produção, sem harness.** Todos os 11 estágios do pipeline renderizados corretamente na `ProgressScreen`; os dois avisos esperados chegaram como eventos (`Modelos de detecção de falantes não instalados`; `A separação de voz falhou — voice-over`); `Arquivo gerado:` mostrou o path certo em `outputsRootDir()` (prova indireta do `onDone`); export SAF via "Salvar em..." gravou os dois MP4s em `/sdcard/Download` com sucesso (`1784307734210_dub_pt.mp4`, `1784308151105_dub_es.mp4`). **Achado extra não planejado:** o job en→es sobreviveu a uma queda completa da conexão `adb`/USB (cabo fisicamente desconectado por ~35 min) — o foreground service e a Activity continuaram rodando sem depender da ponte de debug; ao reconectar, o app já mostrava o job concluído. Fecha formalmente o gate "fixture curta end-to-end nos três idiomas" da §17/D3 (en→pt e en→es cobertos; pt/es→en herdam o mesmo caminho de código, não é uma terceira direção de risco).

**Achado de produção, RESOLVIDO no mesmo dia (2026-07-17, passada separada):** `ModelManager.download()` extraía `.tar.gz` de forma síncrona em Dart puro no isolate principal — para modelos grandes (Whisper "Rápido" 61MB, vozes pt-BR/es 59MB/72MB) isso bloqueava a UI por ~60-180s, tempo suficiente pra disparar o diálogo "app não está respondendo" do Android. Fix de uma linha (`model_manager.dart:1007`): a extração passou a rodar em `Isolate.run` (mesmo padrão já usado no hash-verify do mesmo arquivo). Validado no device com o pior caso — 28 taps consecutivos durante a extração do Whisper Rápido e o download completo do modelo de voz Espanhol (72MB) — zero ANR nos dois. Detalhes (incluindo um achado de borda sobre `Isolate.spawn` travar sem timeout sob pressão extrema de memória do device, não é bug do fix) em `decisoes.md` (2026-07-17, entrada separada da D3.4 acima).

**`models_screen.dart` — rótulo "instalando...":** pra entradas `targz`, `ModelManager.download()` só emite progresso de bytes baixados; o hash-verify e a extração que rodam depois não têm emissão intermediária, e a barra ficava parada em ~100% sem indicar trabalho em andamento (até ~180s no smoke test). Corrigido mostrando "instalando..." quando o progresso reportado chega a ~100% mas o download ainda está na lista ativa.

2 testes novos no engine (371 no total, piso do `verify.ps1` subido de 369→371) + 21 no app (33 no total, incluindo `settings_test.dart` novo). Detalhes completos em `decisoes.md` (2026-07-17).
